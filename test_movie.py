...
def main():
    ...
    movie_list = []
    movie_list.append(Movie(1, "Inception", "Christopher Nolan", 4, True, 5.99))
    movie_list.append(Movie(2, "The Dark Knight", "Christopher Nolan", 0, True, 6.99, 1000))
    movie_list.append(Movie(3, "Inter-Stellar", "Chris Nolan", 5, False, 7.99, 1))
    ...
    current_movie.set_title("Interstellar")
    current_movie.set_director("Christopher Nolan")
    current_movie.set_genre(4)
    current_movie.set_price(4.99)

    # Lastly, print report of all movies (using implicit call to __str__())
    print("\n*** Test 3/Final Output ***")
    print("{:10s}{:30s}{:25s}{:12s}{:12s}{:>12s}{:>12s}".format("ID", "Title",
